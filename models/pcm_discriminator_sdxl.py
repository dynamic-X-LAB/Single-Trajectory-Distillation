...
def modified_forward(
    ...
    if hasattr(self, 'time_embed_act') and self.time_embed_act is not None:
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        if use_motion is True:
            self.modified_forward_func = modified_forward_motion
        else:
            self.modified_forward_func = modified_forward

    def _forward(self, sample, timestep, encoder_hidden_states, added_cond_kwargs):
        features = self.modified_forward_func(
            self.unet,
            sample,
            timestep,
    ...
    def feature_loss(
        ...
        loss = 0.0
        features_fake = self.modified_forward_func(
            self.unet,
            sample_fake,
            timestep,
            encoder_hidden_states,
        ...
        features_real = self.modified_forward_func(
            self.unet,
            sample_real.detach(),
            timestep,
            encoder_hidden_states,
