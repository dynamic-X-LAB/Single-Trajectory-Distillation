...
class DDIMSolverV2:
    ...
    def ddim_style_multiphase(self, pred_x0, pred_noise, timestep_index, multiphase, strength:float=1.0):
        inference_indices = np.linspace(
            0, len(self.ddim_timesteps) * strength, num=multiphase, endpoint=False
        )
    ...
    def ddim_random_s(self, pred_x0, pred_noise, timestep_index, random_range, timestep_index_s=None):

        if timestep_index_s is None:
            timestep_index_s = torch.Tensor([np.random.randint(max(0, int(timestep_index - random_range)), int(timestep_index))]).to(pred_x0.device).long()

        alpha_cumprod_prev = extract_into_tensor(
            self.ddim_alpha_cumprods_prev, timestep_index_s, pred_x0.shape
        )
        dir_xt = (1.0 - alpha_cumprod_prev).sqrt() * pred_noise
        x_prev = alpha_cumprod_prev.sqrt() * pred_x0 + dir_xt

        return x_prev, timestep_index_s
...
def encode_prompt_sdxl(
    ...
    tokenizers = [tokenizer, tokenizer2] if tokenizer is not None else [tokenizer2]
    text_encoders = [text_encoder, text_encoder2] if text_encoder is not None else [text_encoder2]
    ...
    for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
        ...
        prompt_embeds = prompt_embeds.hidden_states[-2] # "2" because SDXL always indexes from the penultimate layer
        prompt_embeds_list.append(prompt_embeds)
    
    return torch.cat(prompt_embeds_list, dim=-1), pooled_prompt_embeds
...
def get_add_time_ids(
    ...
    # fixed values, need to be modified according to the dataset
