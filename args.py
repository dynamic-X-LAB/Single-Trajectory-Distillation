...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="animatediff",
        choices=["animatediff", "animatediff_sdxl", "modelscope"],
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use motion model, if false, distilling by image dataset"
        ),
    )
    parser.add_argument(
        "--use_std",
        ...
        action="store_true",
        help=(
            "Whether to use single trajectory distillation"
        ),
    )
    parser.add_argument(
        "--std_rate",
        ...
        default=0.7,
        help=(
            "the rate selecting from STD bank"
        ),
    )
    parser.add_argument(
        "--solver_mode",
        ...
        default='pcm',
        help=(
            "Whether to use random selected timestep"
        ),
    )
    parser.add_argument(
        '--random_range_rate',
        ...
        default=1.0,
        help=(
            "When using ddim_random_s, `random_range_rate` limit the range of step `s`"
        ),
    )
    # ----------Training Arguments----------
    parser.add_argument("--debug", action="store_true", help="Enable debug mode.")
    parser.add_argument("--sdxl", action="store_true", help="Enable sdxl mode.")
    ...
    parser.add_argument(
        "--using_cfg",
        action="store_true",
        help=("Whether to using cfg, if not, the w_min and w_max are invalid."),
    )
    parser.add_argument(
        "--strength",
        type=float,
        ...
        required=False,
        help=(
            "The strength of adding noise, only add 0~`strength` noise on x0"
        ),
    )
    parser.add_argument(
        "--multiphase",
    ...
    parser.add_argument(
        ...
        type=str,
        default="gt",
        help="The ground truth data for discriminator, one of `target` and `gt`.",
    )
    parser.add_argument(
        "--disc_input_mode",
        type=str,
        default='x0',
        help=(
            "disc input mode, one of `x0`, `xt` and `xs`"
        ),
    )
    # ----------Monitor Arguments----------
    parser.add_argument(
        "--use_monitor",
        action="store_true",
        help="Whether or not to use training monitor.",
    )
    parser.add_argument(
        "--monitor_num",
        type=int,
        default=20,
        help="The number of monitor save num.",
    )
    parser.add_argument(
        "--monitor_steps",
        type=int,
        default=100,
        help="The number of monitor interval.",
    )
    # ----------Other Arguments----------
    parser.add_argument(
        "--remove_motion_lora",
        action="store_true",
        help="Whether or not removing motion lora"
    )
    ...
    elif args.base_model_name == "animatediff_sdxl":
        args.pretrained_teacher_model = (
            "weights/sdxl_base1.0"
        )
        args.motion_adapter_path = (
            "weights/motion_adapter_hsxl"
        )
        logging.info(f'Using `animatediff_sdxl` and load path: {args.pretrained_teacher_model} and {args.motion_adapter_path}')
